...
public class Interaction {
    ...
    public static HashMap<String, Integer> maskMap = new HashMap<String, Integer>() {{
        ...
        put("HELD", 32768);
        put("UNHELD", 65536);
        put("SWAP_OFF_HAND_MAIN_HAND", 131072);
        put("SWAP_OFF_HAND_OFF_HAND", 262144);

        // COMBINATIONS
        put("INVENTORY_SHIFT_CLICK", 16+32);
        put("INVENTORY_LEFT_CLICK_ALL", 1+16);
        put("INVENTORY_RIGHT_CLICK_ALL", 2+32);
        put("SWAP_OFF_HAND", 131072+262144);
        ...
        put("INTERACT", 1+2+4+8+16+32+64+128+256+512+1024+2048+4096+8192+16384+32768+65536+131072+262144);
    }};
    ...
    public boolean isTriggered(BinaryInteraction binaryInteraction) {
        int bitMask = getBitMask(interaction);

        return (binaryInteraction == BinaryInteraction.DROP             && (bitMask & maskMap.get("DROP"))                          != 0)
            || (binaryInteraction == BinaryInteraction.SWAP_TO          && (bitMask & maskMap.get("HELD"))                          != 0)
            || (binaryInteraction == BinaryInteraction.SWAP_FROM        && (bitMask & maskMap.get("UNHELD"))                        != 0)
            || (binaryInteraction == BinaryInteraction.SWAP_MAIN_HAND   && (bitMask & maskMap.get("SWAP_OFF_HAND_MAIN_HAND"))       != 0)
            || (binaryInteraction == BinaryInteraction.SWAP_OFF_HAND    && (bitMask & maskMap.get("SWAP_OFF_HAND_OFF_HAND"))        != 0);
    }
